...
return await bots.handle_combined_scrape(request, background)

@app.post("/scrape/property")
async def scrape_property(request: Request, background: BackgroundTasks):
return await bots.handle_property_scrape(request, background)
...
@app.post("/ai-filter")
async def ai_filter_endpoint(request: Request):
body = await request.json()
